<template>
  <Layout pageTitle="Manage Products" breadcrumbText="" breadcrumbLink="">
    <section id="Products" class="flex flex-col gap-6 flex-1 rounded-3xl p-[18px] px-0 bg-white">
      ...
      <div id="Product-List" class="flex flex-col px-4 gap-5 flex-1">
        ...
        <div v-else-if="error" class="flex flex-col flex-1 items-center justify-center rounded-[20px] border-dashed border-2 border-red-300 gap-6">
          ...
          <button @click="retryFetch" class="btn btn-primary font-semibold">Coba Lagi</button>
        </div>
            ...
            <div class="card flex items-center justify-between gap-3 border-none">
              <div class="flex items-center gap-3 w-[380px] shrink-0">
                ...
                <div class="flex flex-col gap-2 flex-1">
                  ...
                  <p class="font-semibold text-xl text-monday-blue">Rp {{ formatPrice(product.price) }}</p>
                </div>
              </div>
              ...
              <div class="flex items-center gap-4">
                <button @click="viewProduct(product.id)" class="btn btn-primary-opacity min-w-[130px] font-semibold">Details</button>
                <router-link :to="`/products/edit/${product.id}`" class="btn btn-black min-w-[130px] font-semibold">
      ...
      <div v-if="filteredProducts.length > 0" class="flex items-center justify-between px-[18px] py-4">
        <p class="font-medium text-monday-gray">Showing {{ startIndex + 1 }}-{{ endIndex }} of {{ filteredProducts.length }} products</p>
        <div class="flex items-center gap-2">
          <button @click="previousPage" :disabled="currentPage === 1" class="btn btn-primary-opacity font-semibold disabled:opacity-50 disabled:cursor-not-allowed">Previous</button>
          <button
            v-for="page in visiblePages"
            :key="page"
            @click="goToPage(page)"
            :class="['px-4 py-2 rounded-2xl font-semibold transition-300', page === currentPage ? 'bg-monday-blue text-white' : 'bg-monday-gray-background text-monday-gray hover:bg-monday-border']"
          >
            {{ page }}
          </button>
          <button @click="nextPage" :disabled="currentPage === totalPages" class="btn btn-primary-opacity font-semibold disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
        </div>
      </div>
    </section>
  </Layout>
...
export default {
  ...
  setup() {
    ...
    // Cegah empty-state flash: jika produk belum ada, mulai dalam status loading
    // sehingga render pertama langsung tampil spinner, bukan "no data yet"
    if (productsStore.products.length === 0 && !productsStore.error) {
