<template>
  <Layout pageTitle="Add New Product" breadcrumbText="Manage Products" breadcrumbLink="/products">
        ...
        <form @submit.prevent="handleSubmit" class="flex flex-col w-full rounded-3xl p-[18px] gap-5 bg-white">
          ...
          <div class="flex items-center justify-between w-full">
            ...
            <div class="flex flex-col gap-2">
              <button type="button" @click="$refs.fileInput.click()" class="btn btn-black w-[152px] font-semibold text-nowrap">Add Photo</button>
              ...
              <button v-if="selectedFile" type="button" @click="removeImage" class="btn btn-red w-[152px] font-semibold text-nowrap text-sm">Remove Photo</button>
            </div>
          ...
          <label class="group relative rounded-3xl border-[1.5px] border-monday-border focus-within:border-monday-black transition-300 overflow-hidden">
            ...
            <p class="placeholder font-medium text-monday-gray text-sm absolute -translate-y-1/2 left-[81px] top-[25px] group-has-[:invalid]:top-[36px] group-focus-within:top-[25px] transition-300">Select Popularity</p>
            <select v-model="form.popularity" required class="appearance-none w-full h-[72px] font-semibold text-lg outline-none pl-20 pr-6 pb-[14.5px] pt-[32px]">
          ...
          <label class="group relative">
            ...
            <p class="placeholder font-medium text-monday-gray text-sm absolute -translate-y-1/2 left-[81px] top-[25px] group-has-[:placeholder-shown]:top-[36px] group-focus-within:top-[25px] transition-300">Product Name</p>
            <input
              v-model="form.name"
              type="text"
          ...
          <label class="group relative">
            ...
            <p class="placeholder font-medium text-monday-gray text-sm absolute -translate-y-1/2 left-[81px] top-[25px] group-has-[:placeholder-shown]:top-[36px] group-focus-within:top-[25px] transition-300">Product Barcode</p>
            <input
          ...
          <label class="group relative">
            ...
            <p class="placeholder font-medium text-monday-gray text-sm absolute -translate-y-1/2 left-[81px] top-[25px] group-has-[:placeholder-shown]:top-[36px] group-focus-within:top-[25px] transition-300">Product Price</p>
            <input
              v-model="form.price"
              type="number"
              class="appearance-none w-full h-[72px] font-semibold text-lg rounded-3xl border-[1.5px] border-monday-border pl-20 pr-6 pb-[14.5px] pt-[34.5px] placeholder-shown:pt-[14.5px] focus:border-monday-black transition-300"
          ...
          <label class="group relative rounded-3xl border-[1.5px] border-monday-border focus-within:border-monday-black transition-300 overflow-hidden">
            ...
            <p class="placeholder font-medium text-monday-gray text-sm absolute -translate-y-1/2 left-[81px] top-[25px] group-has-[:invalid]:top-[36px] group-focus-within:top-[25px] transition-300">Product Category</p>
            <select v-model="form.category" required class="appearance-none w-full h-[72px] font-semibold text-lg outline-none pl-20 pr-6 pb-[14.5px] pt-[32px]">
          ...
          <label class="flex py-4 px-6 rounded-3xl border-[1.5px] border-monday-border focus-within:border-monday-black transition-300">
            ...
            <div class="flex flex-col gap-[6px] pl-4 w-full">
              <p class="placeholder font-medium text-monday-gray text-sm">Product About</p>
              <textarea v-model="form.description" class="appearance-none outline-none w-full font-semibold text-lg leading-[160%]" rows="5" placeholder="" maxlength="500"></textarea>
              <div class="text-xs text-gray-500 text-right">{{ form.description.length }}/500</div>
            </div>
          </label>
          ...
          <div class="flex items-center justify-end gap-4">
            ...
            <button type="submit" :disabled="loading || imageUploading || !selectedFile" class="btn btn-primary font-semibold">
  ...
  setup() {
    ...
    const validateForm = () => {
      ...
      if (!form.value.barcode.trim()) {
        throw new Error('Barcode produk harus diisi');
      }
